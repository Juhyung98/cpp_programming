...
public:
    ...
    friend ostream& operator <<(ostream& outputStream, const Vector2D& vector);
...
ostream& operator <<(ostream& outputStream, const Vector2D& vector)
{
    outputStream << "(" << vector.vectorX << "," << vector.vectorY << ")";
    return outputStream;
}
